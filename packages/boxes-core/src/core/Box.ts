...
const BOX_EFFECT_MAP = Symbol.for("BOX_EFFECT_MAP")
...
class Box<T> {
  ...
  getTargetEffectMap(target: AnyObj) {
    const conEffWeakMap = this[BOX_EFFECT_MAP]
  ...
  setEffect({
    ...
    if (targetEffectMap === undefined) {
      this[BOX_EFFECT_MAP].set(target, (targetEffectMap = new Map()))
    }
